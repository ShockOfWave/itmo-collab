...
from api.src.utils.paths import (
    get_project_path,
...
from api.src.models.predict_model import (
    get_octane_number_from_model,
    get_prediction_from_segmentation_model,
)
...
def check_models() -> None:
    ...
    if not os.path.exists(save_path):
        ...
        download_file_from_s3(
            s3_uri=os.path.join("weights", "classification_model.ckpt"),
        ...
        download_file_from_s3(
            s3_uri=os.path.join("weights", "segmentation_model.pt"),
        ...
        if not os.path.isfile(os.path.join(save_path, "classification_model.ckpt")):
            ...
            download_file_from_s3(
                s3_uri=os.path.join("weights", "classification_model.ckpt"),
                path_to_local_storage=os.path.join(
                    save_path, "classification_model.ckpt"
                ),
            )
...
def init_models() -> [OctaneClassifier, SegmentationModel]:
    ...
    classification_model = OctaneClassifier.load_from_checkpoint(
        checkpoint_path=PATH_TO_CLASSIFICATION_MODEL, map_location="cuda:0"
    )
