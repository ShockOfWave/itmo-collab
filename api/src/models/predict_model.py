...
from api.src.utils.models_decoders import (
    get_transforms_for_predictions,
    decode_class_names,
)
from api.src.utils.paths import get_project_path


def get_octane_number_from_model(
    model: OctaneClassifier,
    device: str,
...
def get_prediction_from_segmentation_model(
    path_to_image: str = None, image: np.array = None
):
    ...
    model = SegmentationModel(
        model_path=os.path.join(
            get_project_path(), "api", "weights", "segmentation_model.pt"
        )
    )
