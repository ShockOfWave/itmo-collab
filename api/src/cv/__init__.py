...
def create_and_upload_handled_video(video_name):
    ...
    writer = cv2.VideoWriter(
        f"{settings.PROJECT_DIR_PATH}/api/src/upload/{new_video_name_mp4}",
        fourcc,
        fps,
        (width, height),
    )
    ...
    for _, _, files in os.walk(f"{settings.PROJECT_DIR_PATH}/api/src/upload"):
        for file in files:
            upload_file_to_s3(
                f"{settings.PROJECT_DIR_PATH}/api/src/upload/{file}", folder + file
            )
    download_url = create_presigned_s3_url(folder + new_video_name_mp4)
    stream_url = "/".join(
        [settings.S3_URL, settings.S3_BUCKET, new_video_name, new_video_name_m3u8]
    )
